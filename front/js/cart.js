...
// On créer notre variable "cart" dans laquelle on stocke l'élément "cart__items" que l'on récupère dans le DOM. Elle nous servira par la suite à afficher le produit ajouté au panier.
let cart = document.getElementById("cart__items");
// On créer notre fonction "displayAllProductsBasket". Elle va nous permettre d'afficher les produits ajoutés au panier, récupéré dans le local storage.
const displayAllProductsBasket = () => {
    ...
    if(productRegisteredInTheLocalStorage == null) {
        ...
        // On créer notre variable "numberOfProductInBasket" dans laquelle on stocke un tableau vide, afin d'introduire les données de notre produit.
        let numberOfProductInBasket = [];
        // On créer notre boucle for. Elle va parcourir tout les produits de notre tableau "productRegisteredInTheLocalStorage" à chaque tour de boucle.
...
displayAllProductsBasket();

//-------------------- Changer la quantité --------------------//

// On créer notre fonction asynchrone "changeQuantity" on passe en argument la fonction "displayAllProducts". On attend que la fonction "displayAllProducts" s'exécute avant d'exécuter le reste de notre fonction.
...
const changeQuantity = async (displayAllProducts) => {
    ...
    // On créer notre variable "moreQuantity" dans laquelle on stocke tout les éléments "cart__item" que l'on récupère dans le DOM. (querySelectorAll va nous permettre de récupérer tout nos éléments en fonction du nombre de produits ajoutés au panier).  
    ...
    moreQuantity.forEach((selectQuantity) => {
        ...
        selectQuantity.addEventListener("change", (event) => {
            ...
            // On créer notre boucle for. Elle va parcourir les différentes données de notre tableau "productRegisteredInTheLocalStorage" à chaque tour de boucle.
...
// On créer notre fonction "deleteProduct". Elle va nous permettre de supprimer un produit dans notre panier.
const deleteProduct = () => {
// On créer notre variable "toDelete" dans laquelle on stocke notre élément "deleteItem" dans le DOM. (querySelectorAll va nous permettre de récupérer tout nos éléments en fonction du nombre de produits ajoutés au panier). 
let toDelete = document.querySelectorAll(".deleteItem");
console.table(toDelete);
    // On créer notre boucle for. Elle va parcourir le nombre d'élément "deleteItem" du panier à chaque tour de boucle.  
    for(let d = 0; d < toDelete.length; d++) {
        ...
        toDelete[d].addEventListener("click" , () => {
            // On créer notre variable "del" dans laquelle on intègre la methode splice() qui va permettre de supprimer le produit sélectionné par rapport à son index.
...
// On créer notre variable "displayNumber" dans laquelle on stocke l'élément "numberProducts" que l'on récupère dans le DOM.
...
const numberOfReference = () => {
    // Si le nombre de références est inférieur ou égal à O, on affichera "Votre panier est vide !".
    if(productRegisteredInTheLocalStorage.length <= 0) {
        ...
        console.log(number);
    // Sinon si le nombre de références est inférieur ou egale à 1, référence est sans 's'.   
...
// On créer notre fonction "totalCartQuantity". Elle va nous permettre de calculer la quantité total du panier.
const totalCartQuantity = () => {
    // On créer notre variable "tableTotalQuantity" dans laquelle on stocke un tableau vide.
    let tableTotalQuantity = [];
    // On créer notre boucle for. Elle va parcourir toute les valeurs quantity de notre tableau "productRegisteredInTheLocalStorage" à chaque tour de boucle.
    for(let q = 0; q < productRegisteredInTheLocalStorage.length; q++) {
        // On créer notre variable "productsQuantity" dans laquelle on stock la valeur quantity de notre tableau "productRegisteredInTheLocalStorage".
    ...
    // On créer notre fonction "reducerQuantity", dans laquelle on place nos 2 arguments. Le premier va accumuler et stocker notre somme à chaque tour de boucle et le deuxième va être la valeur initiale qui va être additionnée avec l'accumulator à chaque tour de boucle.  
    ...
    // On créer notre variable "displayTotalQuantity" dans laquelle on stocke l'élément "totalQuantity" que l'on récupère dans le DOM.
    let displayTotalQuantity = document.getElementById("totalQuantity");

    // On créer la fonction "numberOfItem". Elle va nous permettre d'afficher le nombre d'articles ajouté au panier, qui sera soumise à une condition. 
    const numberOfItem = () => {
        // Si la quantité totale est inférieure ou egale à O ou si la quantité totale est inférieure ou egale à 1, alors "article" ne prend pas de "s".   
...
// On créer notre fonction "totalCartPrice". Elle nous permettra de calculer le prix total de nos articles dans notre panier.
const totalCartPrice = () => {
    // On créer notre variable "tableTotalPrice" dans laquelle on stocke un tableau vide.
    let tableTotalPrice = [];
    // On créer notre boucle for. Elle va parcourir l'ensemble de nos valeurs price et quantity dans notre tableau "productRegisteredInTheLocalStorage" à chaque tour de boucle par rapport aux produits ajouté au panier.
    for(let p = 0; p < productRegisteredInTheLocalStorage.length; p++) {
        // On créer notre variable "productsQuantity" dans laquelle on stocke la valeur quantity de notre tableau "productRegisteredInTheLocalStorage".
        let productsQuantity = productRegisteredInTheLocalStorage[p].quantity;
        // On créer notre variable "productsPrice" dans laquelle on stocke la valeur price de notre tableau "productRegisteredInTheLocalStorage".
    ...
    // On créer notre fonction "reducerPrice", dans lasquelle on place 2 arguments. Le premier va accumuler et stocker notre somme à chaque tour de boucle et le deuxième va être la valeur initiale qui va être additionnée avec l'accumulator à chaque tour de boucle. 
...
// On créer notre variable "form" dans laquelle on stock l'élément ".cart__order__form" que l'on récupère dans le DOM. Cela va nous permettre de récupérer tout les éléments du formulaire, afin de pouvoir les manipuler facilement. 
...
form.firstName.addEventListener('input', function() {
    ...
    // On créer notre fonction "validFirstName" dans laquelle nous allons introduire une expression régulière (rejex). Elle sera soumise à une condition.
    // Elle va nous permettre de normer la saisi du nom.
    const validFirstName = (inputFirstName) => {
        // On créer notre variable "testFirstName" dans laquelle on stocke notre rejex qui ce compose de 2 slash dans laquelle on place à l'intérieur le symbole "^" correspondant au début de notre séquence et le symbole "$" correspondant à la fin de notre séquence.
        ...
        // On créer notre variable "firstNameErrorMsg" dans laquelle on stocke l'élément "firstNameErrorMsg" que l'on récupère dans le DOM. 
...
form.lastName.addEventListener('input', function() {
    ...
    // On créer notre fonction "validLastName" dans laquelle nous allons introduire une expression régulière (rejex). Elle sera soumise à une condition.
    // Elle va nous permettre de normer la saisi du prénom.
    const validLastName = (inputLastName) => {
        // On créer notre variable "testLastName" dans laquelle on stocke notre rejex qui ce compose de 2 slash dans laquelle on place à l'intérieur le symbole "^" correspondant au début de notre séquence et le symbole "$" correspondant à la fin de notre séquence.
        // Entre crochet on intègre l'ensemble de nos caractères alphabétiques autorisés pour ce champs, puis entre accolade, on détermine le nombre limite de caractéres autorisé, dans notre cas la valeur saisi dans le champs sera limité entre 3 à 20 caractères.
        ...
        // On créer notre variable "lastNameErrorMsg" dans laquelle on stocke l'élément "lastNameErrorMsg" que l'on récupère dans le DOM. 
...
form.address.addEventListener('input', function() {
    ...
    // On créer notre fonction "validAddress" dans laquelle nous allons introduire une expression régulière (rejex). Elle sera soumise à une condition.
    // Elle va nous permettre de normer la saisi de l'adresse.
    const validAddress = (inputAddress) => {
        // On créer notre variable "testAddress" dans laquelle on stocke notre rejex qui ce compose de 2 slash dans laquelle on place à l'intérieur le symbole "^" correspondant au début de notre séquence et le symbole "$" correspondant à la fin de notre séquence.
...
form.city.addEventListener('input', function() {
    ...
    // On créer notre fonction "validCity" dans laquelle nous allons introduire une expression régulière (rejex). Elle sera soumise à une condition.
    // Elle va nous permettre de normer la saisi de la ville. 
    const validCity = (inputCity) => {
        // On créer notre variable "testCity" dans laquelle on stocke notre rejex qui ce compose de 2 slash dans laquelle on place à l'intérieur le symbole "^" correspondant au début de notre séquence et le symbole "$" correspondant à la fin de notre séquence.
        // Entre crochet on intègre l'ensemble de nos caractères alphabétiques autorisés pour ce champs, puis entre accolade, on détermine le nombre limite de caractéres autorisé, dans notre cas la valeur saisi dans le champs sera limité entre 3 à 20 caractères.
...
validEmail(this);
    ...
    // On créer notre fonction "validEmail" dans laquelle nous allons introduire une expression régulière (rejex). Elle sera soumise à une condition.
    // Elle va nous permettre de normer la saisi de l'adresse mail.
    const validEmail = (inputEmail) => {
        // On créer une nouvelle rejex pour notre champs email, que l'on stock dans une variable.
        ...
        // On créer notre variable "testEmail" dans laquelle on stocke notre variable contenant notre rejex.
        ...
        // On créer notre variable "emailErrorMsg" dans laquelle on stocke l'élément "emailErrorMsg" que l'on récupère dans le DOM.
...
// On créer des variables dans laquelles on stocke tout nos éléments de notre formulaire que l'on récupère dans le DOM.
...
sendOrder.addEventListener("click", (event) => {
    event.preventDefault(); // Cela va permettre à l'action par défaut de ne pas être exécutée comme elle l'est normalement.
    // Si les champs du formulaire ne sont pas remplie, alors un message d'alerte s'affichera en nous informant qu'il faut obligatoirement renseigner tout les champs de ce formulaire.
    if(
        !firstName.value ||
        !lastName.value ||
        !address.value ||
    ...
    // Sinon on envoi la commande vers le serveur afin de la valider et d'obtenir notre numéro de commande (orderId).
        ...
        // On créer notre variable "productsOrder" dans laquelle on stocke un tableau vide.
        let productsOrder = [];
        ...
        // On créer notre variable "order" dans laquelle on stocke l'ensemble de nos données formulaire + toutes les "id" de l'ensemble de nos produits du panier.
        const order = {
            contact : {
        ...
        // On créer notre variable "options" dans laquelle on stocke l'entête de la requête
        const options =  {
            method: "POST",
        ...
        fetch("http://localhost:3000/api/products/order", options)
                // On créer une promise .then avec comme argument "response", qui va nous renvoyer nos données au format json.
                .then((response) => response.json())
                // On créer une autre promise .then avec comme argument "data" qui va envoyer notre orderId.  
                .then((data) => {
                console.log(data)
